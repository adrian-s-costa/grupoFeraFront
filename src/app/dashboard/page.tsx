...
import { getCategoryContent, getOneCampaign, getCategoryContentByUserId } from "../../../utils/api/service";
...
export default function SpecificOffer(){
  ...
  useEffect(() => {
    try {
        getCategoryContentByUserId(id!).then((res)=>{
        setCarOffer(res);
      })
    } catch (error) {
      console.error(error)
      ...
      <div className="w-full min-h-screen h-full bg-white p-5 pb-20">
        ...
        <div className="flex flex-col pt-5 gap-5">
          {carOffer && carOffer.map((carro: any, index: any)=>{
            return <div className="flex items-center" onClick={()=>{router.push(`/dashboard/${carro.id}`)}} key={index}>
